...
from paracore import run_cmd, map_cmds, map_func, autotune_from_pilot
...
"""
    ...
    run_parser.add_argument('-e', '--env', action='append', help='Environment variables (KEY=VALUE)')
    run_parser.add_argument('--env-setup', help='Environment setup command')
    run_parser.add_argument('--env-merge', choices=['inherit', 'replace'], default='inherit',
    ...
    run_parser.add_argument('--retry-backoff', type=float, default=30.0, help='Retry backoff (seconds)')
    ...
    batch_parser.add_argument('-e', '--env', action='append', help='Environment variables (KEY=VALUE)')
    batch_parser.add_argument('--env-setup', help='Environment setup command')
    batch_parser.add_argument('--env-merge', choices=['inherit', 'replace'], default='inherit',
                             help='Environment merge strategy')
    ...
    batch_parser.add_argument('--retry-backoff', type=float, default=30.0, help='Retry backoff (seconds)')
    ...
    # Autotune command
    tune_parser = subparsers.add_parser('autotune', help='Run pilot jobs to determine optimal resources')
    tune_parser.add_argument('file', help='File with commands (one per line, use - for stdin)')
    tune_parser.add_argument('-s', '--sample-size', type=int, default=10, help='Number of pilot jobs')
    ...
    tune_parser.add_argument('--memory-guess', type=int, default=8, help='Initial memory guess (GB)')
    tune_parser.add_argument('--time-guess', type=int, default=30, help='Initial time guess (minutes)')
    ...
    tune_parser.add_argument('--export-shell', action='store_true', help='Print shell export commands')
